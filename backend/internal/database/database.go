...
func seedLabSample(ctx context.Context, db *gorm.DB) error {
	...
	items := []model.LabSample{
		...
		{BaseModel: model.BaseModel{Code: "LS-004", Name: "实验室样本示例四", Status: "disposed", Version: 1,
			Description: "已完成检测并按留样期满流程处置的实验室样本记录"}, Facility: "水质检测样本链路审核区域1", Owner: "运行一组",
			Category: "常规", RiskLevel: "low", MetricValue: 8.0, MetricUnit: "unit",
			EffectiveAt: now.Add(-24 * time.Hour), Evidence: "处置联单已归档", RelatedCode: "REL-508-04",
			BatchCode: "SB-003", MethodCode: "AM-003",
			DisposalReason: "检测完成，留样期满按规范处置", DisposedBatchCode: "SB-003", DisposedMethodCode: "AM-003",
