...
type SystemHandler struct {
	...
	samplingBatch service.SamplingBatchService
	labSample     service.LabSampleService
	assayMethod   service.AssayMethodService
	resultReview  service.ResultReviewService
	db            *gorm.DB
	redis         *redis.Client
}

func NewSystemHandler(security service.SecurityService, samplingBatch service.SamplingBatchService, labSample service.LabSampleService, assayMethod service.AssayMethodService, resultReview service.ResultReviewService, db *gorm.DB, redisClient *redis.Client) *SystemHandler {
	return &SystemHandler{security: security, samplingBatch: samplingBatch, labSample: labSample, assayMethod: assayMethod, resultReview: resultReview, db: db, redis: redisClient}
}
...
func (h *SystemHandler) Overview(c *gin.Context) {
	...
	samplingBatchCounts, err := h.samplingBatch.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["sampling-batches"] = samplingBatchCounts

	labSampleCounts, err := h.labSample.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["samples"] = labSampleCounts

	assayMethodCounts, err := h.assayMethod.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["methods"] = assayMethodCounts

	resultReviewCounts, err := h.resultReview.StatusCounts(ctx)
	if err != nil {
		handleError(c, err)
		return
	}
	result["reviews"] = resultReviewCounts
